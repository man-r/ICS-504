...
df = pd.read_csv("./housing-wheelchairs-DFE.csv")

for i, row in df.iterrows():
    if(i<162):
        continue    # continue here 
    print(i, '- id: ', row['_unit_id'], ' lat: ', row['lat'], ' , long: ' ,row['long'])
    #Latitude
    lat=row['lat']
    #Longitude
    lng=row['long']
    ...
    file_name = 'pano_'+str(row['_unit_id'])+'_'+str(newPano.panoid)+".jpg"
    newPano.save(directory=dir,fname=file_name)
